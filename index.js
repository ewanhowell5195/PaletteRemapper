...
const fromCtx = new Canvas(width, height).getContext("2d")
fromCtx.drawImage(fromMapImg, 0, 0)
const fromData = fromCtx.getImageData(0, 0, width, height).data
...
const toMapCtx = new Canvas(width, height).getContext("2d")
toMapCtx.drawImage(toMapImg, 0, 0)
const toMapData = toMapCtx.getImageData(0, 0, width, height).data

for (const file of fs.readdirSync(inputDir)) {
  if (!file.endsWith(".png")) continue

  const donorImg = await loadImage(path.join(inputDir, file))
  const donorCtx = new Canvas(width, height).getContext("2d")
  donorCtx.drawImage(donorImg, 0, 0)
  const donorData = donorCtx.getImageData(0, 0, width, height).data
  ...
  const srcCtx = new Canvas(width, height).getContext("2d")
  ...
  for (const [key] of fromMap) {
    ...
    let baseColor = null
    for (let y = 0; y < height && !baseColor; y++) {
      for (let x = 0; x < width; x++) {
        const i = (y * width + x) * 4
